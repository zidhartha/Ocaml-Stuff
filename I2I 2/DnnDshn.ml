...
| [] -> [] 
| current_stud :: next_stud -> if current_stud.ln = ln then current_stud :: find_by_last_name ln next_stud else find_by_last_name ln next_stud

(* Extended Student Database *)
let rec remove_by_id id db = match db with
| [] -> [] 
| h :: t -> if h.id <> id then h :: remove_by_id id t else remove_by_id id t
